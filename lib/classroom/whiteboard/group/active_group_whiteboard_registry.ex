defmodule Classroom.ActiveGroupWhiteboard.Registry do
  use GenServer

  #API

  def start_link(_args) do
    GenServer.start_link(__MODULE__, nil, name: :group_whiteboard_registry)
  end

  def whereis_name(class_name) do
    GenServer.call(:group_whiteboard_registry, {:whereis_name, class_name})
  end

  def register_name(class_name, pid) do
    GenServer.call(:group_whiteboard_registry, {:register_name, class_name, pid})
  end

  def unregister_name(class_name) do
    IO.puts "3"
    GenServer.cast(:group_whiteboard_registry, {:unregister_name, class_name})
  end
  ...
  def handle_info({:DOWN, _, :process, pid, _}, state) do
   ...
   # that we can use to remove the dead pid from our whiteboard_registry
